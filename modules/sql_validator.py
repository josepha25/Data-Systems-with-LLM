...
class SQLValidator:
    # makes sure queries are safe before running them
    ...
    def _get_query_type(self, statement) -> str:
        # pulls the query type out of the parsed statement
    ...
    def _extract_tables(self, sql: str) -> list:
        # finds table names after FROM and JOIN
        pattern = r'\b(?:FROM|JOIN)\s+([a-zA-Z_][a-zA-Z0-9_]*)'
        matches = re.findall(pattern, sql, re.IGNORECASE)
        return [m.lower() for m in matches]

    def _extract_columns(self, statement) -> list:
        # finds column names in the SELECT clause, skips * and aggregates
        SKIP_KEYWORDS = {"count", "sum", "avg", "min", "max", "distinct"}
        columns = []
        select_seen = False
        ...
        for token in statement.flatten():
            ...
            if select_seen:
                ...
                if val and val not in ("*", ",", " ", "") and val not in SKIP_KEYWORDS:
